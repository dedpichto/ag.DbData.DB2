![Nuget](https://img.shields.io/nuget/v/ag.DbData.DB2)
...
4. Inject IDB2DbDataFactory into your classes:
...
- [ag.DbData.DB2](https://www.nuget.org/packages/ag.DbData.DB2/)
...
```csharp
IServiceCollection AddAgDB2(this IServiceCollection services)
```

Appends the registration of ```IDbDataFactory``` and ```IDbDataObject``` to ```IServiceCollection```.

```csharp
IServiceCollection AddAgDB2(this IServiceCollection services, IConfigurationSection configurationSection)
```
...
```csharp
IServiceCollection AddAgDB2(this IServiceCollection services, Action<DbDataSettings> configureOptions)
```
...
ag.DbData.DB2 is built with the following projects:

- [IBM.Data.DB2.Core](https://www.ibm.com/support/knowledgecenter/en/SSEPGG_10.5.0/com.ibm.swg.im.dbclient.adonet.doc/doc/c0024472.html)
